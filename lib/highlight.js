...
function highlight(text, spans, offset) {
    ...
    return Node(
        ...
        util.morsel(spans).map(function (x) {
            ...
            var attributes = x[2];
            return Node(["." + attributes.slice(0, 1)], {},
...
function highlightLocations(locations, context) {
    ...
    return Node([], {}, items(srcs).map(function (x) {
...
function highlightLocationsSameSource(locations, context) {
    ...
    return Node(
        ...
        [
            Node(["div", ".source"], {}, [
                Node([".sourcefile"], {}, [src.url || "???"]),
                locations.map(function (x) {
                    var loc = x[0];
                    var cls = x[1];
                    return Node(["." + cls, ".sourcepos"], {}, [loc.ref()]);
                })
            ]),
...
function mergeLocations(locs) {
    ...
    if (ll === 0)
        ...
        // var starts = locs.map(function (loc) { return loc.start });
        // var ends = locs.map(function (loc) { return loc.end });
        // return Location(
        //     locs[0].source,
        //     Math.min.apply(null, starts),
        //     Math.min.apply(null, ends)
        // );
    }
}
