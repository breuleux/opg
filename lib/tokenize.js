...
function SimpleTokenizer(config) {
    if (!(this instanceof SimpleTokenizer))
        return new SimpleTokenizer(config);
    this.config = config
    var parts = [];
    this.types = [];
    ...
    // this.re = new RegExp(parts.join("|"), "g");
    this.re = parts.join("|");
}

SimpleTokenizer.prototype.iterator = function(text) {
    var results = this.run(text);
    return results.shift.bind(results);
}

SimpleTokenizer.prototype.run = function(text, index) {
    index = index || 0;
    ...
    while (m = re.exec(text)) {
        ...
        results.push({
            ...
            type: type,
            start: m.index + index,
            end: m.index + m[0].length + index
        });
    }
    results.push({token: "", type: "boundary",
                  start: text.length + index, end: text.length + index});
...
function inferLocation(tok, i, toks) {
    if (tok.start === undefined) {
        tok.start = (toks[i - 1] || {end: 0}).end;
        tok.end = (toks[i + 1] || {start: tok.start}).start || tok.start;
    }
    return tok;
}
